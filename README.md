# TP 1 : Création d'une application React simple (Composants et Props)

## Description
Projet React simple pour le TP MLIAEdu : Création de composants et utilisation de props.  
Ce projet contient les composants suivants :
- `HelloWorld` : Affiche un message simple.
- `Profil` : Affiche un profil utilisateur avec une image et des props.
- `Voiture` : Affiche des informations sur différentes voitures.
- `ListeCourses` : Affiche des listes dynamiques d'éléments.

## Objectif du TP

Découvrir React et son fonctionnement de base.

Créer des composants fonctionnels simples et réutilisables.

Utiliser les props pour passer des données aux composants.

Mettre en pratique PropTypes pour valider les données reçues par les composants.

Apprendre à structurer un projet React avec plusieurs composants et fichiers.

<img width="1060" height="568" alt="Capture d’écran 2026-04-03 200026" src="https://github.com/user-attachments/assets/408dc7dd-66e1-4832-8f67-d13d097614fb" />


<img width="1366" height="768" alt="Capture d’écran 2026-04-03 200220" src="https://github.com/user-attachments/assets/74aa3fa3-0772-419c-b650-1ac012472942" />

